...
# SINGLE HARDCODED WEBHOOK URL FOR ENTIRE APP
WEBHOOK_URL = "https://agentonline-u29564.vm.elestio.app/webhook-test/61e8b566-40c1-4925-940b-c6e74b9563cc"
...
# Show webhook info in sidebar
st.sidebar.markdown("---")
st.sidebar.markdown("### 🎯 Webhook Status")
st.sidebar.success(f"**Active Webhook:**\n{WEBHOOK_URL}")
st.sidebar.info("🔄 Auto-send enabled for all audio recordings")
...
def create_voice_recorder():
    ...
    <div id="voice-recorder" style="text-align: center; padding: 25px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; margin: 20px 0; box-shadow: 0 8px 32px rgba(0,0,0,0.1);">
        ...
        <div style="background: rgba(255,255,255,0.1); padding: 10px; border-radius: 10px; margin-bottom: 20px;">
            <p style="color: white; margin: 0; font-size: 12px;">🎯 Auto-sends to: {WEBHOOK_URL}</p>
        </div>
    ...
    recordBtn.onclick = async () => {{
        try {{
            ...
            mediaRecorder.onstop = async () => {{
                ...
                statusText.innerHTML = "✅ Recording complete! 📤 Auto-sending to n8n...";
...
if page == "🎙️ Voice Chat":
    ...
    # Show webhook info prominently
    st.info(f"🎯 **All audio recordings automatically send to:** `{WEBHOOK_URL}`")
            ...
            if st.button("🚀 Send Uploaded Audio", use_container_width=True):
                with st.spinner("📤 Sending to n8n webhook..."):
                    try:
                        ...
                        success, message = send_to_webhook(payload)
    ...
    with col1:
        if st.button("💬 Send Text Message", use_container_width=True, disabled=not user_name or not prompt_text):
            with st.spinner("📤 Sending text message to n8n webhook..."):
                try:
                    payload = {
                        "timestamp": datetime.now().isoformat(),
                        "user_name": user_name,
...
# PAGE 4: GOOGLE DOCS LIVE
    ...
    if st.session_state.live_content:
        ...
        with col4:
            if st.button("🚀 Send to n8n", use_container_width=True):
                with st.spinner("📤 Sending to n8n webhook..."):
                    payload = {
                        "timestamp": datetime.now().isoformat(),
...
# PAGE 5: SETTINGS
    ...
    st.markdown("### 🎯 n8n Webhook Configuration")
    st.success(f"**Active Webhook URL:** `{WEBHOOK_URL}`")
    st.info("This webhook is hardcoded and handles all audio recordings automatically.")
    
    # Test webhook
    if st.button("🧪 Test n8n Webhook", use_container_width=True):
        with st.spinner("🧪 Testing n8n webhook connection..."):
            test_payload = {
                "test": True,
                "timestamp": datetime.now().isoformat(),
                "message": "Test connection from Book Buddy Settings",
                "app_name": "Book Buddy",
    ...
    with col1:
        ...
        st.info("🔄 Auto-send is always enabled for voice recordings")
...
<div style='text-align: center; color: #666; padding: 20px;'>
    ...
    <p>🎯 <strong>n8n Webhook:</strong> {WEBHOOK_URL}</p>
</div>
